...
func.currentConfig = function(callback) {
  ...
  config.backgroundColour = $('#backgroundColour').val();
  // / Background Color
...
var configureUI = function() {
  func.getSettings(function(settings) {
    ...
    if (settings.backgroundColour) {
      $('#backgroundColour').val(settings.backgroundColour);
      $('.mdc-text-field__icon.background-colour').css('color',settings.backgroundColour);
    }
    // / Background Color
...
$(document).ready(function () {
  ...
  // Andrei: Background Color
  $('.background-colour').materialColorPicker({
    mode: "modal",
  	format: "rgba", // Format that allows transparency
  	opacity: true, // Set to "true" to parse transparency
  	onSelect: function(e){
  		if (e.currentcolor){
  			var color = e.currentcolor.color;
  			$('#backgroundColour').val(color);
        $('.mdc-text-field__icon.background-colour').css('color',color);
        func.currentConfig(function(config) {
          func.saveSettings(config, function(settings) {
  ...
  $('#toolbarColour').on('keyup', function() {
    ...
    // Andrei: Toolbar Color
    $('.mdc-text-field__icon.toolbar-colour').css('color',$('#toolbarColour').val());
    // / Toolbar Color
    func.currentConfig(function(config) {
      func.saveSettings(config, function(settings) {
        console.log("Toolbar colour set");
  ...
  $('#backgroundColour').on('keyup', function() {
    $('.mdc-text-field__icon.background-colour').css('color',$('#backgroundColour').val());
    func.currentConfig(function(config) {
      func.saveSettings(config, function(settings) {
        console.log("Background colour set");
      });
...
function resetSettings() {
  ...
  // Andrei: Background Color
  config.backgroundColour = "rgba(0, 0, 0, 0.5)";
  // / Background Color
